...
const StationButton = React.forwardRef<HTMLButtonElement, StationButtonProps>(({ station, onClick, selectedStation, children }, ref) => {
  return (
    <button 
      onClick={onClick} 
      ref={ref} 
      className={cn("shadow-lg rounded-xl p-2 transition-all",station === selectedStation ? 'bg-black dark:bg-white not-dark:text-white dark:text-black scale-110' : 'bg-black/50 dark:bg-white/50 not-dark:text-white dark:text-black')}
